...
const UziDiagnosisEdit: React.FC<UziDiagnosisEditProps> = ({
    ...
    const [addNode, { isLoading: isAddingNode, error: errorAddNode }] =
        useAddNodeMutation()
    ...
    const [
        deleteSegment,
        { isLoading: isDeletingSegment, error: errorDeleteSegment },
    ] = useDeleteSegmentMutation()
    const [
        changeSegment,
        { isLoading: isChangingSegment, error: errorChangeSegment },
    ] = useChangeSegmentMutation()
    const [
        changeNodeTirads,
        { isLoading: isChangingNodesTirads, error: errorChangeNodeTirads },
    ] = useChangeNodeTiradsMutation()
    ...
    useRTKEffects(
        { isLoading: isDeletiongNode, error: errorDeleteNode },
        DELETE_NODE
    )
    useRTKEffects(
        { isLoading: isAddingSegment, error: errorAddSegment },
        ADD_SEGMENT
    )
    useRTKEffects(
        { isLoading: isDeletingSegment, error: errorDeleteSegment },
        ...
        nodes.forEach((node) => {
            ...
            if (node.exist) {
                ...
                if (segments.length > 0 && !node.toDelete) {
                    addNode({
                        uziId: uziId,
                        payload: { node: otherData, segments: segments },
                    })
                } else if (segments.length === 0) {
                    dispatch(
                        addError({
            ...
            dispatch(
                addCurrentNodes(
                    res.map(({ node, segments }) => ({
                        ...
                        tirads_23:
                            node.tirads === 'tirads_23' ? node.tiradsValue : 0,
                        tirads_4:
                            node.tirads === 'tirads_4' ? node.tiradsValue : 0,
                        tirads_5:
                            node.tirads === 'tirads_5' ? node.tiradsValue : 0,
                        exist: true,
                    }))
                )
            )
