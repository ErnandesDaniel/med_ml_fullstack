...
const UziAddNode: React.FC<UziAddNodeProps> = ({
    form,
    closeModal,
    editMode = false,
}) => {
    ...
    const handleAddNode = () => {
        if (form.getFieldsValue().node) {
            dispatch(
                addNode({
                    tirads_23:
                        form.getFieldsValue().node === 'tirads_23' ? 1 : 0,
        ...
        if (selectedNode) {
            dispatch(
                changeNodeTirads({
                    ...
                    tirads: form.getFieldsValue().node,
                })
            )
    ...
    return (
        <Form
            layout="vertical"
            form={form}
            onFinish={editMode ? handleEditNode : handleAddNode}
        >
            <Form.Item
                name="node"
                label="Выберите тип нового узла"
                rules={[{ required: true }]}
            >
                <Select>
